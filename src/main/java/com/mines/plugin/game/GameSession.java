...
public class GameSession {
    ...
    public boolean isRevealed(int index) { return revealedSafe.contains(index); }

    /** Returns current multiplier based on safe tiles revealed.
     *  Uses a mathematically fair approach: each safe reveal multiplies the
     *  running multiplier by (totalSafe - revealedSoFar) / (totalCells - revealedSoFar),
     *  inverted and scaled by a house edge factor (0.97). Mirrors how Stake calculates it.
     */
    public double getCurrentMultiplier(Map<Integer, Double> multiplierTable) {
        int revealed = revealedSafe.size();
        if (revealed == 0) return 1.0;
        double houseEdge = 0.97; // 3% house edge
        double multiplier = 1.0;
        int safe = totalCells - mineCount;
        for (int i = 0; i < revealed; i++) {
            double prob = (double)(safe - i) / (double)(totalCells - i);
            multiplier *= houseEdge / prob;
        }
        return multiplier;
    }
